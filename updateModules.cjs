...
if (modules.length === 0) {
  console.error('No modules provided. Please provide a list of modules as command line arguments.');
  process.exit(1);
}
...
fs.readFile(filePath, 'utf8', (err, data) => {
  ...
  const result = data.replace(regex, `${placeholderStart} ${replacement} ${placeholderEnd}`);
